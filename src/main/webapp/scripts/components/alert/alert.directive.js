...
angular.module('mountyhubApp')
    .directive('jhAlert', function(AlertService) {
        return {
            ...
            template: '<div class="alerts" ng-cloak="">' +
                            '<div ng-repeat="alert in alerts | filter:{type:\'success\'}" ng-class="[alert.position, {\'toast\': alert.toast}]">' +
    ...
    .directive('jhAlertError', function(AlertService, $rootScope) {
        return {
            restrict: 'E',
            template: '<div class="alerts" ng-cloak="">' +
                            '<div ng-repeat="alert in alerts | filter:{type:\'danger\'}" ng-class="[alert.position, {\'toast\': alert.toast}]">' +
            ...
            controller: ['$scope',
                function($scope) {
                    ...
                    var cleanHttpErrorListener = $rootScope.$on('mountyhubApp.httpError', function (event, httpResponse) {
                        var i;
                        ...
                        switch (httpResponse.status) {
                            ...
                            case 400:
                                var errorHeader = httpResponse.headers('X-mountyhubApp-error');
                                var entityKey = httpResponse.headers('X-mountyhubApp-params');
                                if (errorHeader) {
